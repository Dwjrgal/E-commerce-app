...
const Email = () => {
  ...
  const handleSendOtp = async () => {
    console.log(email);
    try {
      const res = await axios.post( "http://localhost:8000/api/v1/auth/recovery-pass",
        { email }
      );
      if(res.status === 200) {
        setStep(step +1)
  ...
  return (
    <div className="flex flex-col gap-3 items-center h-[800px] pt-40 bg-slate-100">
      ...
      <div className="flex flex-col gap-4">
        ...
        />
        <button className="w-72 h-8 border bg-blue-600 rounded-full text-white" onChange={handleSendOtp}>
          Илгээх
        </button>
      </div>
      </>
